...
var _ref = require('@breush/ref-napi');
  ...
  function ArrayType(data, length) {
    ...
    if (0 === arguments.length) {
      ...
      if (data.length != expectedLength) {
        if (data.length < expectedLength) {
          throw new Error(
            'buffer length must be at least ' + expectedLength + ', got ' + data.length,
          );
        } else {
...
function slice(start, end) {
  ...
  if (end) {
    ...
    data = this.buffer.slice(
      start * this.constructor.BYTES_PER_ELEMENT,
      end * this.constructor.BYTES_PER_ELEMENT,
    );
  } else {
